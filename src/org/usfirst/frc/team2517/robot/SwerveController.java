...
public class SwerveController {
	...
	public SwerveController(int moveMotor, int turnMotor, boolean invertedY,
							...
							int moveBR, int turnBR, int encBR) throws FileNotFoundException, UnsupportedEncodingException{
		try{ // TODO: Fix reading of files
			...
			calIn.close();
		}catch(IOException ex){ // Create new file here with blank offsets and use
			PrintWriter writer = new PrintWriter(CALFILE, "UTF-8");
			for(int i = 0; i<swerves.length; i++){
				offsets.add(0.0);
				writer.println("0.0");
			}
